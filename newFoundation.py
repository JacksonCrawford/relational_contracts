...
def scraper(link):
    ...
    soupyLinks = soup.find(class_="article-main-component__content")
    ...
    try:
        author = soupyLinks.find(class_="byline-component__content")
        aTag = author.find("a")
    ...
    try:
        date = soupyLinks.find(class_="date-mdy").contents[0]
        time = soupyLinks.find(class_="date-gia").contents[0]
        timestamp = str(date) + " " + str(time)
        jsonDict["timestamp"] = timestamp.lower()
    except AttributeError:
    ...
    try:
        chunkySoup = soupyLinks.find(class_="article-body-component article-body-component--null")
